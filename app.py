...
if uploaded_file:
    ...
    # --- BOTTOM ROW: Analytics Visualization ---
    st.markdown("---")
    ...
    with vis_col2:
            ...
            # --- Updated Font Colors to White ---
            ax_radar.set_xticks(angles[:-1])
            # Setting xticklabels (Categories) to white
            ax_radar.set_xticklabels(categories, color='white', size=10, fontweight='bold')
            
            ax_radar.set_ylim(0, 10)
            ax_radar.set_yticks([2, 4, 6, 8, 10])
            # Setting yticklabels (Numbers 2-10) to white
            ax_radar.set_yticklabels(["2", "4", "6", "8", "10"], color='white', size=8)
            
            # Make the circular grid lines slightly brighter for contrast
            ax_radar.grid(color="#33334d", linestyle='--')
            
            # Adjust the spine (outer circle) color
