...
if uploaded_file:
    ...
    with vis_col1:
        ...
        fig_bar, ax_bar = plt.subplots(figsize=(8, 5), facecolor='
        ax_bar.set_facecolor('
        y_pos = np.arange(len(categories))
        colors = ["
        ax_bar.barh(y_pos, values, color=colors, edgecolor="
        ax_bar.set_yticks(y_pos)
        ax_bar.set_yticklabels(categories, color='
        ax_bar.set_xlim(0, 10)
        ax_bar.xaxis.grid(True, color='
        ax_bar.set_axisbelow(True)
        for i, v in enumerate(values):
            ax_bar.text(v + 0.2, i, str(v), color='
    ...
    with vis_col2:
            ...
            fig_radar = plt.figure(figsize=(6, 6), facecolor='
            ax_radar = fig_radar.add_subplot(111, polar=True)
            ax_radar.set_facecolor('
            ax_radar.set_theta_offset(np.pi / 2)
            ax_radar.set_theta_direction(-1)
            ax_radar.plot(angles, vals_radar, color='
            ax_radar.fill(angles, vals_radar, color='
            ...
            ax_radar.grid(color="
            ax_radar.spines['polar'].set_color('
            st.pyplot(fig_radar)
else:
